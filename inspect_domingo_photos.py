...
async def fetch_page(session, page_num):
    url = f"https://www.hepsiburada.com/domingo-yayinevi-hot-ile-zot-iki-magara-adaminin-hikayesi-alastair-chisholm-p-HBCV0000AKVV1A-yorumlari?sayfa={page_num}"
    headers = {
    ...
    for i, card in enumerate(top_cards):
        ...
        has_photo = len(imgs) > 0 or h64 > 0 or w80 > 0
...
async def main():
    async with AsyncSession() as session:
        ...
        print(f"\n==================================================")
        print(f"TOTAL REVIEW CARDS ACROSS PAGES 1-4: {tot_cards}")
        print(f"TOTAL WRITTEN TEXT REVIEWS ACROSS PAGES 1-4: {tot_text_cards}")
        print(f"TOTAL PHOTO REVIEW CARDS ACROSS PAGES 1-4: {tot_photo_cards}")
        print(f"==================================================")
